...
void *Dispatcher::_dispatch(void *_arg) {
	arg_t arg = (arg_t) _arg;
	arg->dispatcher->dispatch(*arg->dispatcher->m_vDevices[arg->index]);
	delete arg;
}
...
void Dispatcher::run() {
	...
	for (size_t i = 0 ; i < m_vDevices.size(); i++) {
		...
		arg->index = i;
		arg->dispatcher = this;
		pthread_create(&thread, NULL, _dispatch, arg);
...
void Dispatcher::init() {
	std::cerr << "Initializing devices..." << std::endl;
	std::cerr << std::endl;
...
void Dispatcher::initContinue(Device & d) {
	...
	std::cerr << "Initializing " << d.m_index << " Remaining: " << sizeLeft << '\r';
...
void Dispatcher::onEvent(cl_int status, Device & d) {
...
void CL_CALLBACK Dispatcher::staticCallback(cl_event event, cl_int event_command_exec_status, void * user_data) {
	...
	pDevice->m_parent.onEvent(event_command_exec_status, *pDevice);
}
